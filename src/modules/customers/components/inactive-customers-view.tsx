...
import { buildInactiveSummaryCards } from "../utils/customers-segment-summary";
...
export function InactiveCustomersView() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await customersSegmentService.getInactiveCustomers();
  ...
  const summaryItems = useMemo(() => buildInactiveSummaryCards(filtered, t), [filtered, t]);
    ...
    <div className="zelify-workspace-page">
      ...
      <div className="zelify-workspace-page__scroll">
        <div className="zelify-workspace-page__inner">
          <header>
            <h1 className="zelify-workspace-page__title">{t("customers.inactive.title")}</h1>
            <p className="zelify-customer-segment-page__subtitle">{t("customers.inactive.subtitle")}</p>
          </header>
          ...
          <div className="zelify-customer-segment-page__search">
            <AppInput
              type="search"
              variant="surface"
              placeholder={t("customers.inactive.searchPlaceholder")}
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              autoComplete="off"
              aria-label={t("customers.inactive.searchAria")}
            />
          </div>
          ...
          <div className="zelify-customer-segment-page__filters">
            <CustomerSegmentFilters variant="inactive" />
          </div>
          ...
          <div className="zelify-customer-segment-page__table">
            {loading ? (
              <div className="zelify-workspace-page__loading">
                ...
                <span>{t("customers.inactive.loading")}</span>
              </div>
            ) : (
              <>
                <InactiveCustomersTable rows={filtered} />
                {filtered.length > 0 ? (
